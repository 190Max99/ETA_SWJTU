...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     IPS200   清屏函数(限制区域)
// 参数说明     void
// 返回参数     void
// 使用示例     tft180_clear_region();
// 备注信息     将屏幕清空成背景颜色
//-------------------------------------------------------------------------------------------------------------------
void ips200_clear_region(uint16 x1, uint16 y1, uint16 dis_width, uint16 dis_value_max)
{
    uint16 color_buffer[160];
    uint16 i = 0, j = 0;

    IPS200_CS(0);
    ips200_set_region(x1, y1, x1 + dis_width - 1, y1 + dis_value_max - 1);
